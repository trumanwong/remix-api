...
font = ImageFont.truetype("/data/docker/golang/remix-api/scripts/static/fonts/jianti.ttf", font_size)
...
def generate_image(text, dst):
    origin_im = Image.open("/data/docker/golang/remix-api/scripts/static/images/nokia.png")
    text = text[:900]
    length = len(text)
    width, height = font.getsize(text)
    current_width = 0
    ...
    origin_im.save(dst, "png")  # Save as local file
    f = Image.open(dst)
    f = f.resize((int(f.size[0] / 5), int(f.size[1] / 5)), Image.ANTIALIAS)
    f.save(dst, optimize=True, quality=90)
